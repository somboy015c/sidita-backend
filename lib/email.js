...
async function sendCustomerMessageEmail(customer, messageBody, senderName) {
  const html = wrapper(`
    <h2 style="margin:0 0 4px; font-size:19px;">New message from SIDITA</h2>
    <p style="color:#5C6B66; font-size:13.5px; margin:0 0 16px;">${senderName} sent you a message:</p>
    <div style="background:#F4F5F6; border-radius:10px; padding:16px; font-size:14px; white-space:pre-wrap;">${messageBody}</div>
    <p style="margin-top:18px; font-size:13px; color:#5C6B66;">Sign in to your SIDITA account to reply.</p>
  `);
  await sendEmail({ to: customer.email, subject: `New message from SIDITA`, html });
}

async function sendAdminMessageNotifyEmail(customer, messageBody) {
  const admins = await getAllAdminEmails();
  if (!admins.length) return;
  const html = wrapper(`
    <h2 style="margin:0 0 4px; font-size:19px;">New message from ${customer.name}</h2>
    <p style="color:#5C6B66; font-size:13.5px; margin:0 0 16px;">${customer.email}${customer.phone ? ' · ' + customer.phone : ''}</p>
    <div style="background:#F4F5F6; border-radius:10px; padding:16px; font-size:14px; white-space:pre-wrap;">${messageBody}</div>
    <p style="margin-top:18px; font-size:13px; color:#5C6B66;">Reply from the Requests page in the admin panel.</p>
  `);
  await sendEmail({ to: admins, subject: `New message from ${customer.name}`, html });
}
